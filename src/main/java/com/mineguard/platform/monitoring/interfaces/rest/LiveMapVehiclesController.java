...
@RestController
@RequestMapping(value = "/api/v1/vehicles/positions", produces = MediaType.APPLICATION_JSON_VALUE)
@Tag(name = "Vehicle Positions", description = "Real-time GPS positions of the company fleet. " +
        "Position data is a state sub-resource of Vehicle: each entry enriches the vehicle record " +
        "with current coordinates, speed, heading, and connection status as reported by the on-board GPS unit. " +
        "The path /vehicles/positions models this as a specialized sub-collection of the vehicles resource. " +
        "Formerly GET /vehicles/live-positions.")
public class LiveMapVehiclesController {
    ...
    @GetMapping
    @Operation(
            summary = "List current positions for all fleet vehicles",
